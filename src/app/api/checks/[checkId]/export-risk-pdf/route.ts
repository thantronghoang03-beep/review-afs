...
import { generateRiskPdfReport } from "@/lib/export/risk-pdf-report";
...
export async function GET(_req: Request, ctx: Params) {
  ...
  const buffer = await generateRiskPdfReport(check, check.riskAnalysis);

  return new NextResponse(new Uint8Array(buffer), {
    headers: {
      "Content-Type": "application/pdf",
      "Content-Disposition": `attachment; filename="phan-tich-rui-ro-${checkId}.pdf"`,
    },
  });
}
